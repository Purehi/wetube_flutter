...
class StreamClient {
  ...
  Future<StreamManifest> getManifest(dynamic videoId,
    ...
    final watchPage = requireWatchPage
        ? await WatchPage.get(_httpClient, videoId.value, accessToken: accessToken)
        : null;
      ...
      try {
        await retry(_httpClient, () async {
          final streams = await _getStreams(videoId,
                  ytClient: client, watchPage: watchPage, accessToken: accessToken)
              .toList();
  ...
  Stream<StreamInfo> _getStreams(VideoId videoId,
      ...
      required WatchPage? watchPage, String? accessToken}) async* {
    // Use await for instead of yield* to catch exceptions
    await for (final stream
        in _getStream(videoId, ytClient, watchPage, accessToken: accessToken)) {
      yield stream;
    }
  }

  Stream<StreamInfo> _getStream(VideoId videoId, YoutubeApiClient ytClient,
      WatchPage? watchPage, {String? accessToken}) async* {

    final playerResponse = await _controller
        .getPlayerResponse(videoId, ytClient, watchPage: watchPage, accessToken: accessToken);
