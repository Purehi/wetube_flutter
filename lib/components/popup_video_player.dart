...
import 'package:you_tube/pages/youtube_template_page.dart';
...
class _PopupVideoPlayerState extends State<PopupVideoPlayer>{
  ...
  void initState() {
    ...
    if(_playerController != null){
      if(_playerController?.value.isInitialized != true){
        ...
        if(widget.video.recommends.isNotEmpty && !isLive){
  ...
  void _listener(){
    ...
    final total = _playerController?.value.duration.inSeconds ?? 0;
    if((total - position) < 10){
      if(_currentIndex + 1 < widget.video.recommends.length){
        final tmpCurrentIndex = _currentIndex + 1;
        final item = widget.video.recommends[tmpCurrentIndex];
    ...
    if(_playerController?.value.isCompleted == true){
      if(_currentIndex + 1 < widget.video.recommends.length){
        _currentIndex += 1;
        final item = widget.video.recommends[_currentIndex];
        _changedVideo(item);
      }
    }
  }
    ...
    if(currentIndexKey == 0){
      if(navigatorKey.currentContext != null){
        Navigator.of(navigatorKey.currentContext!, rootNavigator: true).push(MaterialPageRoute(
          builder: (_) => YoutubeTemplatePage(video: _video, _playerController),));
        PictureInPicture.stopPiP();
      }
    }
